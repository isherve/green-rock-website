  ...
  return (
    <div className={cn("relative", className)}>
      <select
        value={locale}
        onChange={(e) => setLocale(e.target.value as LocaleCode)}
        className="appearance-none rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 pl-8 pr-8 py-2 text-sm font-medium cursor-pointer hover:border-primary/40 focus:outline-none focus:ring-2 focus:ring-primary/30"
        aria-label={translate("footerLanguage", locale)}
      >
        {LOCALES.map((loc) => (
          <option key={loc.code} value={loc.code}>
